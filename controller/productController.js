...
module.exports.createProduct = async (req, res) => {
  ...
  await product.save().catch((err) => res.sendStatus(400));
...
module.exports.deleteProduct = async (req, res) => {
  ...
  await Product.findByIdAndDelete(id).catch((err) => res.sendStatus(400));
...
module.exports.putProduct = async (req, res) => {
  ...
  try {
    ...
    res.status(400).json({ message: "Can not change product" });
  }
  res.status(200).json({
